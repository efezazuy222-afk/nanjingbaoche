...
const coverLine = /^cover\s*:\s*.*$/m;
if (!coverLine.test(text)) throw new Error("生成内容缺少封面图字段。");
text = text.replace(coverLine, `cover: "${cover}"`);
if (type === "blog") {
  const dateLine = /^date\s*:\s*.*$/m;
  if (!dateLine.test(text)) throw new Error("生成内容缺少发布日期字段。");
  text = text.replace(dateLine, `date: "${today}"`);
}
